...
def job_detail(request, job_id):
    ...
    return render(request, 'job_detail_1.html', {'job': job})
...
@login_required(login_url='login_company')
...
def add_job(request):
    if request.method == 'POST':
        ...
        try:
            company = Company.objects.get(user=request.user)  # Fetch the company linked to the user
        except Company.DoesNotExist:
